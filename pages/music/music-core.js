...
import { settingsManager } from '../../src/core/settings.js';
    ...
    function noteToMidi(note) {
        ...
        const accidentalOffset = accidental === "#" || accidental === "s" ? 1 : accidental === "b" ? -1 : 0;
    ...
    function renderRangeOptions(selectedIndex = 0) {
        ...
        const safeSelectedIndex = Number.isInteger(parsedIndex) && RANGE_OPTIONS[parsedIndex] ? parsedIndex : 0;
    ...
    function loadUserSettings() {
        try {
            ...
            if (settings) {
                return {
                    audio: settings.audio || { volume: 0.8, noteDelay: 400, answerDelay: 1000, autoPlayNext: true },
                    game: { startingDifficulty: settings.music?.startingDifficulty || 0, melodyLength: settings.music?.melodyLength || 3, pointsPerCorrect: settings.music?.pointsPerCorrect || 10, showHints: settings.music?.showHints || true },
    ...
    function initMultiNoteTrainingListeners() {
        ...
        function checkAnswer() {
            if (userSelection.length !== currentMelody.length) {
                showInlineResult(resultDisplay, `还需要选择 ${currentMelody.length - userSelection.length} 个音符`, 'warning');
                return;
            }
            ...
            if (correctCount === currentMelody.length) {
                ...
                melodyScore = 0;
                scoreDisplay.textContent = melodyScore;
                showInlineResult(resultDisplay, `答对 ${correctCount}/${currentMelody.length}，连击已重置`, 'error');
            }
        }
    ...
    function init() {
        debugLog('初始化音乐模块...');
        if (!initAudioContext()) { debugError('音频上下文初始化失败'); return; }
    ...
    function addTouchFeedback() {
        // 针对所有可交互元素添加触摸反馈
        const interactiveElements = document.querySelectorAll('.note-button, .scale-button, .play-button, .music-card, .selected-note, .delete-note');
