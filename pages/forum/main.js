...
async function init() {
    ...
    if (hash === 'blog') {
        ...
        appState.currentMode = storage.getString(StorageKeys.forumMode, 'forum');
    }
    ...
    if (changeRepoBtn) {
        changeRepoBtn.addEventListener('click', () => {
            ...
            if (newOwner) {
                ...
                if (newRepo) {
                    storage.setJSON(StorageKeys.forumRepo, { owner: newOwner, name: newRepo });
...
async function switchMode(newMode) {
    ...
    storage.setString(StorageKeys.forumMode, newMode);
...
async function handleIssueClick(issueNumber) {
    ...
    try {
        // 加载Issue详情
        await issues.loadIssueDetail(issueNumber);
