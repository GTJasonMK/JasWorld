...
import { aiConfigManager, DEFAULT_AI_CONFIG, AI_PROVIDER_DEFAULTS } from '@shared/llm/config.js';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function applyAIProviderPreset(provider) {
    ...
    if (
      apiUrlInput &&
      (!apiUrlInput.value.trim() || apiUrlInput.value.trim() === previousPreset.apiUrl)
    ) {
      apiUrlInput.value = preset.apiUrl;
    }
    if (
      modelInput &&
      (!modelInput.value.trim() || modelInput.value.trim() === previousPreset.model)
    ) {
  ...
  function loadAISettings(config = aiConfigManager.getWithDefaults()) {
    ...
    if ($('ai-provider'))
      $('ai-provider').dataset.currentProvider = config.provider || DEFAULT_AI_CONFIG.provider;
  ...
  function loadSettings() {
    ...
    try {
      ...
      document.body.classList.toggle(
        'light-theme',
        resolveThemeChoice(settings.ui?.theme) === 'light'
      );
      ...
      setChecked(
        ...
        settings.ui?.highContrast,
        DEFAULT_SETTINGS.ui.highContrast
      );
      ...
      setText(
        $('note-delay-slider-value'),
        settings.audio?.noteDelay,
        DEFAULT_SETTINGS.audio.noteDelay
      );
      setIf(
        $('answer-delay-slider'),
        settings.audio?.answerDelay,
        DEFAULT_SETTINGS.audio.answerDelay
      );
      setText(
        $('answer-delay-slider-value'),
        settings.audio?.answerDelay,
        DEFAULT_SETTINGS.audio.answerDelay
      );
      setChecked(
        $('auto-play-next'),
        settings.audio?.autoPlayNext,
        DEFAULT_SETTINGS.audio.autoPlayNext
      );
      setChecked(
        $('sound-effects-toggle'),
        settings.audio?.soundEffects,
        DEFAULT_SETTINGS.audio.soundEffects
      );
      setChecked(
        $('use-synth-audio-toggle'),
        settings.audio?.useSynthAudio,
        DEFAULT_SETTINGS.audio.useSynthAudio
      );

      setIf(
        $('starting-difficulty'),
        settings.music?.startingDifficulty,
        DEFAULT_SETTINGS.music.startingDifficulty
      );
      setIf($('melody-length'), settings.music?.melodyLength, DEFAULT_SETTINGS.music.melodyLength);
      setIf(
        $('points-per-correct'),
        settings.music?.pointsPerCorrect,
        DEFAULT_SETTINGS.music.pointsPerCorrect
      );
      setText(
        $('points-per-correct-value'),
        settings.music?.pointsPerCorrect,
        DEFAULT_SETTINGS.music.pointsPerCorrect
      );
      ...
      setText(
        ...
        settings.games?.gameSpeed,
        DEFAULT_SETTINGS.games.gameSpeed
      );
      setChecked(
        $('vibration-feedback-toggle'),
        settings.games?.vibrationFeedback,
        DEFAULT_SETTINGS.games.vibrationFeedback
      );
      ...
      setChecked(
        $('show-avatars-toggle'),
        settings.forum?.showAvatars,
        DEFAULT_SETTINGS.forum.showAvatars
      );
      setIf(
        $('comments-per-page'),
        settings.forum?.commentsPerPage,
        DEFAULT_SETTINGS.forum.commentsPerPage
      );
      ...
      setIf(
        $('tetris-base-speed'),
        settings.tetris?.baseLevelSpeed,
        DEFAULT_SETTINGS.tetris.baseLevelSpeed
      );
      setText(
        $('tetris-base-speed-value'),
        settings.tetris?.baseLevelSpeed,
        DEFAULT_SETTINGS.tetris.baseLevelSpeed
      );
      setIf(
        $('tetris-speed-decrease'),
        settings.tetris?.speedDecreasePerLevel,
        DEFAULT_SETTINGS.tetris.speedDecreasePerLevel
      );
      setText(
        $('tetris-speed-decrease-value'),
        settings.tetris?.speedDecreasePerLevel,
        DEFAULT_SETTINGS.tetris.speedDecreasePerLevel
      );
      setIf(
        $('tetris-score-multiplier'),
        settings.tetris?.scoreMultiplierIncrement,
        DEFAULT_SETTINGS.tetris.scoreMultiplierIncrement
      );
      setText(
        $('tetris-score-multiplier-value'),
        settings.tetris?.scoreMultiplierIncrement,
        DEFAULT_SETTINGS.tetris.scoreMultiplierIncrement
      );
      setIf(
        $('tetris-combo-multiplier'),
        settings.tetris?.comboMaxMultiplier,
        DEFAULT_SETTINGS.tetris.comboMaxMultiplier
      );
      setText(
        $('tetris-combo-multiplier-value'),
        settings.tetris?.comboMaxMultiplier,
        DEFAULT_SETTINGS.tetris.comboMaxMultiplier
      );
      setIf(
        $('tetris-ability-threshold'),
        settings.tetris?.abilityThresholdAdjustment,
        DEFAULT_SETTINGS.tetris.abilityThresholdAdjustment
      );
      setText(
        $('tetris-ability-threshold-value'),
        settings.tetris?.abilityThresholdAdjustment,
        DEFAULT_SETTINGS.tetris.abilityThresholdAdjustment
      );
      setIf(
        $('tetris-special-piece'),
        settings.tetris?.maxSpecialPieceChance,
        DEFAULT_SETTINGS.tetris.maxSpecialPieceChance
      );
      setText(
        $('tetris-special-piece-value'),
        settings.tetris?.maxSpecialPieceChance,
        DEFAULT_SETTINGS.tetris.maxSpecialPieceChance
      );
      ...
      setIf(
        $('snake-initial-speed'),
        settings.snake?.initialSpeed,
        DEFAULT_SETTINGS.snake.initialSpeed
      );
      setText(
        $('snake-initial-speed-value'),
        settings.snake?.initialSpeed,
        DEFAULT_SETTINGS.snake.initialSpeed
      );
      setIf(
        $('snake-speed-decrease'),
        settings.snake?.speedDecreasePerPoint,
        DEFAULT_SETTINGS.snake.speedDecreasePerPoint
      );
      setText(
        $('snake-speed-decrease-value'),
        settings.snake?.speedDecreasePerPoint,
        DEFAULT_SETTINGS.snake.speedDecreasePerPoint
      );
      setIf(
        $('snake-special-food'),
        settings.snake?.specialFoodChance,
        DEFAULT_SETTINGS.snake.specialFoodChance
      );
      setText(
        $('snake-special-food-value'),
        settings.snake?.specialFoodChance,
        DEFAULT_SETTINGS.snake.specialFoodChance
      );
      setIf(
        $('snake-food-multiplier'),
        settings.snake?.specialFoodScoreMultiplier,
        DEFAULT_SETTINGS.snake.specialFoodScoreMultiplier
      );
      setText(
        $('snake-food-multiplier-value'),
        settings.snake?.specialFoodScoreMultiplier,
        DEFAULT_SETTINGS.snake.specialFoodScoreMultiplier
      );
      setChecked(
        $('snake-wall-collision'),
        settings.snake?.wallCollision,
        DEFAULT_SETTINGS.snake.wallCollision
      );

      setIf(
        $('2048-animation-speed'),
        settings.game2048?.animationSpeed,
        DEFAULT_SETTINGS.game2048.animationSpeed
      );
      setText(
        $('2048-animation-speed-value'),
        settings.game2048?.animationSpeed,
        DEFAULT_SETTINGS.game2048.animationSpeed
      );
      const bonusStep =
        settings.game2048?.levelBonusStep ?? DEFAULT_SETTINGS.game2048.levelBonusStep;
      ...
      setIf(
        $('2048-tile4-probability'),
        settings.game2048?.initialTile4Probability,
        DEFAULT_SETTINGS.game2048.initialTile4Probability
      );
      setText(
        $('2048-tile4-probability-value'),
        settings.game2048?.initialTile4Probability,
        DEFAULT_SETTINGS.game2048.initialTile4Probability
      );
      setChecked(
        $('2048-animations-toggle'),
        settings.game2048?.showAnimations,
        DEFAULT_SETTINGS.game2048.showAnimations
      );

      setIf(
        $('memory-default-difficulty'),
        settings.memory?.defaultDifficulty,
        DEFAULT_SETTINGS.memory.defaultDifficulty
      );
      setIf(
        $('memory-card-duration'),
        settings.memory?.cardShowDuration,
        DEFAULT_SETTINGS.memory.cardShowDuration
      );
      setText(
        $('memory-card-duration-value'),
        settings.memory?.cardShowDuration,
        DEFAULT_SETTINGS.memory.cardShowDuration
      );
      setIf(
        $('memory-match-bonus'),
        settings.memory?.consecutiveMatchBonus,
        DEFAULT_SETTINGS.memory.consecutiveMatchBonus
      );
      setText(
        $('memory-match-bonus-value'),
        settings.memory?.consecutiveMatchBonus,
        DEFAULT_SETTINGS.memory.consecutiveMatchBonus
      );
      setIf(
        $('memory-wrong-penalty'),
        settings.memory?.wrongMatchPenalty,
        DEFAULT_SETTINGS.memory.wrongMatchPenalty
      );
      setText(
        $('memory-wrong-penalty-value'),
        settings.memory?.wrongMatchPenalty,
        DEFAULT_SETTINGS.memory.wrongMatchPenalty
      );
      setIf(
        $('memory-timer-penalty'),
        settings.memory?.timerPenaltyPerSecond,
        DEFAULT_SETTINGS.memory.timerPenaltyPerSecond
      );
      setText(
        $('memory-timer-penalty-value'),
        settings.memory?.timerPenaltyPerSecond,
        DEFAULT_SETTINGS.memory.timerPenaltyPerSecond
      );
    } catch (e) {
  ...
  function resetToDefaults() {
    ...
    setIf($('font-size-slider'), d.ui.fontSize);
    setText($('font-size-slider-value'), d.ui.fontSize);
    ...
    resetAIToDefaults();
    setIf($('volume-slider'), d.audio.volume);
    setText($('volume-slider-value'), d.audio.volume);
    setIf($('note-delay-slider'), d.audio.noteDelay);
    setText($('note-delay-slider-value'), d.audio.noteDelay);
    setIf($('answer-delay-slider'), d.audio.answerDelay);
    setText($('answer-delay-slider-value'), d.audio.answerDelay);
    ...
    setIf($('points-per-correct'), d.music.pointsPerCorrect);
    setText($('points-per-correct-value'), d.music.pointsPerCorrect);
    setChecked($('show-hints'), d.music.showHints);
    setIf($('game-speed-slider'), d.games.gameSpeed);
    setText($('game-speed-slider-value'), d.games.gameSpeed);
    ...
    // 游戏特定
    setIf($('tetris-base-speed'), d.tetris.baseLevelSpeed);
    setText($('tetris-base-speed-value'), d.tetris.baseLevelSpeed);
    setIf($('tetris-speed-decrease'), d.tetris.speedDecreasePerLevel);
    setText($('tetris-speed-decrease-value'), d.tetris.speedDecreasePerLevel);
    setIf($('tetris-score-multiplier'), d.tetris.scoreMultiplierIncrement);
    setText($('tetris-score-multiplier-value'), d.tetris.scoreMultiplierIncrement);
    setIf($('tetris-combo-multiplier'), d.tetris.comboMaxMultiplier);
    setText($('tetris-combo-multiplier-value'), d.tetris.comboMaxMultiplier);
    setIf($('tetris-ability-threshold'), d.tetris.abilityThresholdAdjustment);
    setText($('tetris-ability-threshold-value'), d.tetris.abilityThresholdAdjustment);
    setIf($('tetris-special-piece'), d.tetris.maxSpecialPieceChance);
    setText($('tetris-special-piece-value'), d.tetris.maxSpecialPieceChance);
    setIf($('snake-initial-speed'), d.snake.initialSpeed);
    setText($('snake-initial-speed-value'), d.snake.initialSpeed);
    setIf($('snake-speed-decrease'), d.snake.speedDecreasePerPoint);
    setText($('snake-speed-decrease-value'), d.snake.speedDecreasePerPoint);
    setIf($('snake-special-food'), d.snake.specialFoodChance);
    setText($('snake-special-food-value'), d.snake.specialFoodChance);
    setIf($('snake-food-multiplier'), d.snake.specialFoodScoreMultiplier);
    setText($('snake-food-multiplier-value'), d.snake.specialFoodScoreMultiplier);
    setChecked($('snake-wall-collision'), d.snake.wallCollision);
    setIf($('2048-animation-speed'), d.game2048.animationSpeed);
    setText($('2048-animation-speed-value'), d.game2048.animationSpeed);
    setIf($('2048-level-bonus'), d.game2048.levelBonusStep);
    setText($('2048-level-bonus-value'), d.game2048.levelBonusStep);
    setIf($('2048-tile4-probability'), d.game2048.initialTile4Probability);
    setText($('2048-tile4-probability-value'), d.game2048.initialTile4Probability);
    setChecked($('2048-animations-toggle'), d.game2048.showAnimations);
    setIf($('memory-default-difficulty'), d.memory.defaultDifficulty);
    setIf($('memory-card-duration'), d.memory.cardShowDuration);
    setText($('memory-card-duration-value'), d.memory.cardShowDuration);
    setIf($('memory-match-bonus'), d.memory.consecutiveMatchBonus);
    setText($('memory-match-bonus-value'), d.memory.consecutiveMatchBonus);
    setIf($('memory-wrong-penalty'), d.memory.wrongMatchPenalty);
    setText($('memory-wrong-penalty-value'), d.memory.wrongMatchPenalty);
    setIf($('memory-timer-penalty'), d.memory.timerPenaltyPerSecond);
    setText($('memory-timer-penalty-value'), d.memory.timerPenaltyPerSecond);
  }
  ...
  function saveSettings() {
    const settings = {
      ...
      games: {
        ...
        vibrationFeedback:
          $('vibration-feedback-toggle')?.checked ?? DEFAULT_SETTINGS.games.vibrationFeedback,
      ...
      forum: {
        ...
        commentsPerPage: parseInt(
          $('comments-per-page')?.value || DEFAULT_SETTINGS.forum.commentsPerPage
        ),
        defaultSort: $('default-sort')?.value || DEFAULT_SETTINGS.forum.defaultSort,
      },
      tetris: {
        baseLevelSpeed: parseInt(
          $('tetris-base-speed')?.value || DEFAULT_SETTINGS.tetris.baseLevelSpeed
        ),
        speedDecreasePerLevel: parseInt(
          $('tetris-speed-decrease')?.value || DEFAULT_SETTINGS.tetris.speedDecreasePerLevel
        ),
        minSpeed: DEFAULT_SETTINGS.tetris.minSpeed,
        scoreMultiplierBase: DEFAULT_SETTINGS.tetris.scoreMultiplierBase,
        scoreMultiplierIncrement: parseFloat(
          $('tetris-score-multiplier')?.value || DEFAULT_SETTINGS.tetris.scoreMultiplierIncrement
        ),
        maxScoreMultiplier: DEFAULT_SETTINGS.tetris.maxScoreMultiplier,
        comboMaxMultiplier: parseFloat(
          $('tetris-combo-multiplier')?.value || DEFAULT_SETTINGS.tetris.comboMaxMultiplier
        ),
        ...
        maxSpecialPieceChance: parseFloat(
          $('tetris-special-piece')?.value || DEFAULT_SETTINGS.tetris.maxSpecialPieceChance
        ),
      },
      snake: {
        initialSpeed: parseInt(
          $('snake-initial-speed')?.value || DEFAULT_SETTINGS.snake.initialSpeed
        ),
        minSpeed: DEFAULT_SETTINGS.snake.minSpeed,
        speedDecreasePerPoint: parseFloat(
          $('snake-speed-decrease')?.value || DEFAULT_SETTINGS.snake.speedDecreasePerPoint
        ),
        ...
        specialFoodChance: parseFloat(
          $('snake-special-food')?.value || DEFAULT_SETTINGS.snake.specialFoodChance
        ),
        specialFoodScoreMultiplier: parseFloat(
          $('snake-food-multiplier')?.value || DEFAULT_SETTINGS.snake.specialFoodScoreMultiplier
        ),
      ...
      game2048: {
        ...
        levelBonusMultipliers: generateBonusMultipliers(
          parseFloat($('2048-level-bonus')?.value || DEFAULT_SETTINGS.game2048.levelBonusStep)
        ),
        levelBonusStep: parseFloat(
          $('2048-level-bonus')?.value || DEFAULT_SETTINGS.game2048.levelBonusStep
        ),
        initialTile4Probability: parseFloat(
          $('2048-tile4-probability')?.value || DEFAULT_SETTINGS.game2048.initialTile4Probability
        ),
        ...
        animationSpeed: parseInt(
          $('2048-animation-speed')?.value || DEFAULT_SETTINGS.game2048.animationSpeed
        ),
        showAnimations:
          $('2048-animations-toggle')?.checked ?? DEFAULT_SETTINGS.game2048.showAnimations,
      },
      memory: {
        difficulties: DEFAULT_SETTINGS.memory.difficulties,
        cardShowDuration: parseInt(
          $('memory-card-duration')?.value || DEFAULT_SETTINGS.memory.cardShowDuration
        ),
        timerPenaltyPerSecond: parseInt(
          $('memory-timer-penalty')?.value || DEFAULT_SETTINGS.memory.timerPenaltyPerSecond
        ),
        consecutiveMatchBonus: parseInt(
          $('memory-match-bonus')?.value || DEFAULT_SETTINGS.memory.consecutiveMatchBonus
        ),
        wrongMatchPenalty: parseInt(
          $('memory-wrong-penalty')?.value || DEFAULT_SETTINGS.memory.wrongMatchPenalty
        ),
        defaultDifficulty:
          $('memory-default-difficulty')?.value || DEFAULT_SETTINGS.memory.defaultDifficulty,
      },
    };
  ...
  async function testOpenAICompatibleConnection(config) {
    ...
    const result = await client.stream([{ role: 'user', content: '请只回复 OK' }], {
      timeout: 30,
  ...
  function activateSettingsTab(targetId, updateHash = false) {
    ...
    document
      .querySelectorAll('.settings-tabs .tab-button')
  ...
  document.querySelectorAll('.games-subtabs .subtab-button').forEach((tab) => {
    tab.addEventListener('click', function () {
      document
        .querySelectorAll('.games-subtabs .subtab-button')
        .forEach((t) => t.classList.remove('active'));
